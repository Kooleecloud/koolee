...
| **Position ping**   | `driver_position_pings`                       | **Append-only, short retention.** Every fix a phone reported, kept for days rather than forever. The mutable row above answers "where is my driver now" and destroys the previous answer doing it; this answers "did we lose them, for how long, and on whose phone" — which nothing could answer before it existed. Also **not** chain of custody. |
