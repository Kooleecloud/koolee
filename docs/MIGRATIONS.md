...
| 0035      | `luxuriant_sphinx`         | 2026-09-01 | `driver_shifts.started_by_user_id` — the admin who opened a shift on a driver's behalf, `ON DELETE set null`. Additive column, no backfill                                                                                                                                                                                               |
| 0036      | `redundant_felicia_hardy`  | 2026-09-06 | `driver_position_pings` — append-only diagnostics for driver GPS gaps, with the device fix time and the server write time side by side. Pure CREATE TABLE: no lock on an existing table beyond the FK's, no scan, one `DROP TABLE` to reverse. ⚠️ **Highest-volume write in the system**; the retention sweep ships with it              |
