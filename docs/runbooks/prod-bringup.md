...
| D2 | **`{{ .Token }}` in three templates** — *Confirm signup*, *Magic Link*, *Change Email Address*. A template holding only `{{ .ConfirmationURL }}` sends a link, and the funnel asks for a code. | TD |
...
⚠️ J9's header carries an unclosed obligation: *re-verify each coverage area
against the drive-time assumptions the cutoff maths relies on — an out-of-area
booking that gets sold is a booking that misses its flight.*
