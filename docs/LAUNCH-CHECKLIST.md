...
- [ ] ⚠️ **Promoting `0033` to `main` carries a booking-creation window for the
      ...
      discover it.** The two options — accept the window, or sequence it by
      hand behind `coming_soon` — are written out as
      [prod-bringup A9](runbooks/prod-bringup.md), which cites
      [MIGRATIONS §9.5](MIGRATIONS.md#0033-broke-this-and-rode-the-workflow-anyway).
      **No migration fixes this and none should be written:** splitting an
      applied migration changes its content hash and puts every database that
      has it into permanent drift.
...
| L4  | **At least one agreement version published** — v2, with the pinning language, after legal review. **Zero versions is a total outage of the pickup flow:** no booking can hold an acceptance, so the identity gate fails closed and every agent visit stops at the door. There is deliberately NO boot gate on this — a day-zero production deploy has to come up before v1 can be written in it — so the console raises the alarm instead, on Overview and on `/agreements` | TD    | draft prepared ([launch/agreement-v2-draft.md](launch/agreement-v2-draft.md)); needs counsel, then `/agreements` |
...
| Item                                                                                                                                                                        | What closed it                                                                                                                                                                                                                    |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Nothing checked a PR** — `migrate.yml` was the whole of CI, so a branch could arrive red on typecheck, lint, tests, builds and formatting and nothing said so             | `.github/workflows/ci.yml`: format, typecheck, lint, unit tiers, 3 prod builds, and the core integration tier against an ephemeral Postgres. `migrate.yml` untouched and still the only workflow that can reach a hosted database |
| **The agreement renderer showed raw markdown** — no branch for `#`, links or tables, and `parseAgreementMarkdown` degrades the unrecognised to paragraph text               | All three built end to end, AST first. `safeLinkHref` allow-lists `http`/`https`/`mailto` in the AST, so every consumer gets the same answer                                                                                      |
| **No way to READ a published agreement** without opening it in `amend`, one click from publishing a new version                                                             | A read-only version view on `/agreements`, drawn by the same `<Markdown>` the customer sees                                                                                                                                       |
| **Zero published versions was an invisible outage** — the customer saw an empty "Action needed", the agent was told the customer had not accepted, the console said nothing | `no_agreement_published` is its own gate blocker; a calm customer holding state with the passport step OPEN; a console banner on Overview and `/agreements`                                                                       |
| **A refused ZIP cost the customer their whole form** and returned them to the upload door                                                                                   | `rejectedEntrySchema` — a quarantined per-step draft key, so a refusal costs a correction                                                                                                                                         |
| **A cancelled booking was still a stop on the agent's day**, with a working "Start & navigate"                                                                              | `JobState.cancelled` off the BOOKING status; no actions; the stop stays visible, struck through                                                                                                                                   |
| **`reserved_spaces` was never enforced** — four readers each computed capacity raw                                                                                          | One `bookableSpaces()`, four readers, plus a `reserved < capacity` guard on write                                                                                                                                                 |
| **`custody_events` was a pre-staged silent realtime failure**                                                                                                               | `0034` removes it from the publication — the reverse of the expected fix, and why (MIGRATIONS §6)                                                                                                                                 |
| **The console could end a shift but not start one**                                                                                                                         | `adminStartShiftOnBehalf` + `0035`'s `started_by_user_id`, calling the same `startShift` the driver's own button runs                                                                                                             |
| **The console could move a pickup but not remove the driver**                                                                                                               | `adminUnassignPickup`, refused once the bags are in the van                                                                                                                                                                       |
| **One tile error killed the customer's map permanently**                                                                                                                    | An error is fatal only before `load`; `data-map-state` on the DOM for the next diagnosis                                                                                                                                          |
| **Two integration tests depended on another suite's fixture** and would have gone red on CI's first fresh container                                                         | The cutoff row is seeded in its own file's `beforeEach`                                                                                                                                                                           |
...
| ~~P3~~  | ~~**`reserved_spaces` enforcement**~~ — **shipped.** It was never one subtraction in one function: FOUR readers computed capacity independently (the shortlist filter, the candidate it renders, the transactional recheck under the advisory lock, the console's reassign picker), so a reserve honoured in three of four would have been a race no test could see. One `bookableSpaces()` now, plus a `reserved < capacity` guard on write. See RUN-REPORT-13 |
| ~~P4~~  | ~~**A map on the customer trip page**~~ — **shipped.** MapLibre over OpenFreeMap: no key, no account, no per-load billing. The reasoning that deferred it was half right — a distance answers "how long", and a moving pin answers "is anything happening". See RUN-REPORT-14                                                                                                                                                                                   |
...
| ~~P15~~ | ~~**`custody_events` SELECT grant**~~ — **closed the other way (migration 0034).** The table LEFT the realtime publication instead. Nothing subscribes and nothing should — the doorbell is `booking_signals` — and the coverage was not what the note assumed: one policy, not two, with no staff half. Granting would have been a privilege change with no feature behind it. See MIGRATIONS §6                                                               |
...
| B3  | **The tab-closed push delivery matrix**                            | A manual pass in a real Chrome — Chrome-for-Testing subscribes to FCM _preprod_           | A human with a real browser and a real device                                                              |
