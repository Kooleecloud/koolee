...
describeIntegration(
  "phone/email upgrade guard — acceptance tests 15 & 16 (integration)",
  () => {
    ...
    it("test 15: an abandoned anonymous claimant is removed before the new session verifies, so only the new session ends up holding the phone", async () => {
      ...
      expect(
        await db.query.users.findFirst({ where: eq(users.id, a.userId) }),
      ).toBeUndefined();
    ...
    it("test 16 (phone): returns PHONE_EXISTS before any send, then the conflict branch reparents the draft and deletes the orphaned anonymous user", async () => {
      ...
      const anonAfterConflict = await db.query.users.findFirst({
        where: eq(users.id, anon.userId),
      });
      ...
      const { error: signInErr } = await signInClient.auth.signInWithOtp({
        phone: PHONE,
      });
      ...
      await reparentBookingDraft(db, {
        fromUserId: anon.userId,
      ...
      expect(
        await db.query.users.findFirst({ where: eq(users.id, anon.userId) }),
      ).toBeUndefined();
    ...
    it("merged guard: two overlapping guarded sends for one destination serialize, removing a stale claimant exactly once", async () => {
      ...
      expect([...ga.removedAnonymousUserIds, ...gb.removedAnonymousUserIds]).toEqual([
        c.userId,
      ]);
