...
describeIntegration(
  "staff auth — role guard, invite, password reset (integration)",
  () => {
    ...
    it("requireStaffRole blocks customers, anonymous sessions, wrong roles, and deactivated staff — and allows the matching role", async () => {
      ...
      await expect(requireStaffRole(db, customer!.id, ["agent"])).rejects.toThrow(
        NotAuthorizedError,
      );
      await expect(requireStaffRole(db, customer!.id, ["admin"])).rejects.toThrow(
        NotAuthorizedError,
      );
