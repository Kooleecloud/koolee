...
| Migrations "succeed" but the local DB is empty | They went to a different database. Env precedence: shell `DIRECT_DATABASE_URL` > shell `DATABASE_URL` > `DIRECT_DATABASE_URL` from the first file that sets it > `DATABASE_URL` from the first file that sets it — files searched in the order `packages/db/.env.local`, `packages/db/.env`, root `.env.local`, root `.env`. So a `DIRECT_DATABASE_URL` in _any_ file beats a `DATABASE_URL` in _any_ file — and `packages/db/.env` points at the **cloud** project. Run `pnpm test:env:doctor` to see which source wins. |
