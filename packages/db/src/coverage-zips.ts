/**
 * Service-area ZIP data — the single source read by BOTH sides:
 * `@koolee/core`'s coverage logic (isInCoverage / checkCoverage / areas) and
 * this package's seed (dev agent-zone distribution). The data lives here
 * because core depends on db, never the reverse — a seed importing core
 * would be a workspace cycle.
 *
 * Import via the `@koolee/db/coverage-zips` subpath, NOT the package root:
 * this module is pure data with zero imports, so client bundles that reach it
 * through core's coverage module never pull the Postgres driver.
 *
 * The boundary itself is still a commercial decision that changes rarely and
 * is reviewed in a diff — see `packages/core/src/coverage/nyc-zips.ts` for
 * the operational caveats (drive-time re-verification before real sales).
 */

/** Manhattan. */
export const MANHATTAN_ZIPS = [
...
/** Staten Island. */
export const STATEN_ISLAND_ZIPS = [
...
export const HUDSON_COUNTY_NJ_ZIPS = [
  "07002", "07029", "07030", "07032", "07047", "07086", "07087", "07093",
  "07094", "07302", "07304", "07305", "07306", "07307", "07310", "07311",
] as const;

/** Every covered ZIP, flat. Order: area blocks as declared above. */
export const ALL_COVERAGE_ZIPS: readonly string[] = [
  ...MANHATTAN_ZIPS,
  ...BROOKLYN_ZIPS,
  ...QUEENS_ZIPS,
  ...BRONX_ZIPS,
  ...STATEN_ISLAND_ZIPS,
  ...HUDSON_COUNTY_NJ_ZIPS,
];
