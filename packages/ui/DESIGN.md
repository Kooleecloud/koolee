...
packages/ui/styles/theme.css   tokens: color, type scale, spacing, shadows, container
        │
packages/ui  (primitives)      Button, Input, Select, Card, Badge, Dialog, Toaster…
        │
packages/ui  (shell)           AppHeader, ContentColumn, AppFooter, PageHeader, BackLink
        │
packages/ui  (feedback)        Spinner, FormMessage, EmptyState, Skeleton/PageSkeleton,
        │                      ConfirmDialog, toast, BookingStatusBadge, EnvStatusCard
        │
...
- **Content**: `ContentColumn` — one width per surface type:
  `default` (max-w-3xl) customer/admin pages · `narrow` (max-w-md) auth forms
  and the phone-first agent app · `full` dense admin tables.
  Rhythm is fixed: `py-10` page padding, `gap-6` between blocks.
...
## Brand rules (from BRAND.md and koolee/brand/)
...
4. Derive favicons/PWA tiles from `koolee/brand/app-tile.svg` (masked
...
`pnpm --filter @koolee/ui storybook` — the living catalog of every primitive,
shell piece, and feedback state. New component → new story in the same PR.
